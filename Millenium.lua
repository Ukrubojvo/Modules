    ...
    -- Misc functions
            ...
            frame.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                    ...
                    frame_start = vec2(
                        ...
                        frame.AbsolutePosition.Y + frame.AbsoluteSize.Y * frame.AnchorPoint.Y + gui_offset
                    )
                    input_object = input
                end
            end)
        ...
        function library:update_config_list() 
            ...
            for idx, file in listfiles(game_config_path) do
                local name = file:match("([^/\\]+)%.cfg$") or file:match("([^/\\]+)$")
                if name and name ~= "" and not name:find("autoload") then
                    list[#list + 1] = name
                end
            end
        ...
        function library:get_auto_load_config()
            ...
            if isfile(auto_load_path) then
                return readfile(auto_load_path)
            end
            
            return nil
        end

        function library:set_auto_load_config(config_name)
            local auto_load_path = library.directory .. "/configs/" .. library.game_id .. "/autoload.txt"
        ...
        function library:auto_load_config()
            local config_name = library:get_auto_load_config()
        ...
        function library:section(properties)
            ...
            local items = cfg.items; do 
                ...
                items[ "inline" ] = library:create( "Frame" , {
                    ...
                    Size = dim2(1, -2, 1, -2);
                    BorderSizePixel = 0;
                    BackgroundColor3 = rgb(16, 16, 20)  -- NEW: --surface-2
                });
            ...
            if cfg.fading_toggle then
                ...
                function cfg.toggle_section(bool)
                    library:tween(items[ "toggle" ], {BackgroundColor3 = bool and themes.preset.accent or rgb(27, 27, 27)}, Enum.EasingStyle.Quad)
                    library:tween(items[ "toggle_outline" ], {BackgroundColor3 = bool and themes.preset.accent or rgb(27, 27, 27)}, Enum.EasingStyle.Quad)
        ...
        function library:toggle(options) 
            ...
            local items = cfg.items; do
                ...
                if cfg.info then 
                    items[ "info" ] = library:create( "TextLabel" , {
                        FontFace = fonts.small;
                        TextColor3 = rgb(120, 120, 120);  -- NEW: text-sub
                        BorderColor3 = rgb(0, 0, 0);
                        TextWrapped = true;
                        Text = cfg.info;
                        Parent = items[ "toggle" ];
            ...
            function cfg.set(bool)
                ...
                if cfg.type == "checkbox" then 
                    ...
                    library:tween(items[ "toggle_button" ], {BackgroundColor3 = bool and themes.preset.accent or rgb(27, 27, 27)})
                    library:tween(items[ "outline" ], {BackgroundColor3 = bool and themes.preset.accent or rgb(16, 16, 20)})
                else
                    library:tween(items[ "toggle_button" ], {BackgroundColor3 = bool and themes.preset.accent or rgb(27, 27, 27)}, Enum.EasingStyle.Quad)
                    library:tween(items[ "inline" ], {BackgroundColor3 = bool and themes.preset.accent or rgb(27, 27, 27)}, Enum.EasingStyle.Quad)
            ...
            function cfg.set_visible(bool)
                if bool then
                    items["outline"].Position = dim_offset(
                        items["tick"].AbsolutePosition.X - 270 + items["tick"].AbsoluteSize.X,
            ...
            function cfg.refresh_options(options_to_refresh)
                ...
                for _, option_data in options_to_refresh do
                    ...
                    name.MouseEnter:Connect(function()
                        ...
                        library:tween(name, {TextColor3 = rgb(200, 200, 200)})
                    end)
        ...
        function library:init_config(window) 
            ...
            config_holder = section:list({
                ...
                callback = function(option) 
                    ...
                    library.config_flags["config_name_text"](option)
                end, 
            ...
            local section2 = column2:section({name = "Settings", size = 1, default = true, icon = "rbxassetid://75339943202126"})
            ...
            section2:button({name = "Load Config", confirm = true, callback = function() 
                ...
                if not config_name or config_name == "" then
                    notifications:create_notification({
                        name = "Error", 
                        info = "Please select or enter a config name"
                    })
                    return
                end
