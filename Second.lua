...
if not isfolder("AntiLua/Config") then
    makefolder("AntiLua/Config")
end
...
local ConfigFile = "AntiLua/Config/Data_" .. gameName .. ".json"
...
function CircleClick(Button, X, Y)
    spawn(function()
        ...
        Circle.Image = "rbxassetid://124526064259356"
...
function RcHub:MakeNotify(NotifyConfig)
    ...
    NotifyConfig.Color = NotifyConfig.Color or Color3.fromRGB(204, 0, 0)
...
function rcchub(msg, delay, color, title, desc)
    return RcHub:MakeNotify({
        ...
        Color = color or Color3.fromRGB(204, 0, 0),
        Delay = delay or 4
    })
end
...
function RcHub:Window(GuiConfig)
    ...
    local RCglob = Instance.new("ScreenGui");
    local DropShadowHolder = Instance.new("Frame");
    local DropShadow = Instance.new("ImageLabel");
    ...
    RCglob.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    RCglob.Name = "RCglob"
    RCglob.ResetOnSpawn = false
    RCglob.Parent = game:GetService("CoreGui")
    ...
    DropShadowHolder.ZIndex = 0
    DropShadowHolder.Name = "DropShadowHolder"
    DropShadowHolder.Parent = RCglob

    DropShadowHolder.Position = UDim2.new(0, (RCglob.AbsoluteSize.X // 2 - DropShadowHolder.Size.X.Offset // 2), 0,
        (RCglob.AbsoluteSize.Y // 2 - DropShadowHolder.Size.Y.Offset // 2))
    ...
    function GuiFunc:DestroyGui()
        if CoreGui:FindFirstChild("RCglob") then
            RCglob:Destroy()
        end
    end
    ...
    Close.Activated:Connect(function()
        ...
        Yes.MouseButton1Click:Connect(function()
            if RCglob then RCglob:Destroy() end
